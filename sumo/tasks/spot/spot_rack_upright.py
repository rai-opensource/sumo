...
from sumo.tasks.spot.spot_rack_push import SpotRackPush
...
@dataclass
class SpotRackUprightConfig(SpotUprightConfig):
    """Configuration for Sumo's simplified Spot rack upright analysis task."""


class SpotRackUpright(SpotRackPush):
    """Task getting Spot to upright a randomly oriented tire rack."""

    name: str = "spot_rack_upright"
    config_t: type[SpotRackUprightConfig] = SpotRackUprightConfig  # type: ignore[assignment]
    config: SpotRackUprightConfig

    def __init__(self, config: SpotRackUprightConfig | None = None) -> None:
    ...
    def success(self, model: MjModel, data: MjData, metadata: dict[str, Any] | None = None) -> bool:
        """Check if the tire rack is upright."""
...
__all__ = ["SpotRackUpright", "SpotRackUprightConfig"]
