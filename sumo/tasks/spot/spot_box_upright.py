...
from sumo.tasks.spot.spot_box_push import SpotBoxPush
...
class SpotBoxUprightConfig(SpotUprightConfig):
    """Configuration for Sumo's simplified Spot box upright analysis task."""


class SpotBoxUpright(SpotBoxPush):
    """Task getting Spot to upright a randomly oriented box."""

    name: str = "spot_box_upright"
    config_t: type[SpotBoxUprightConfig] = SpotBoxUprightConfig  # type: ignore[assignment]
    ...
    def reset_pose(self) -> np.ndarray:
        """Reset pose with a random box attitude that clears the ground."""
        object_pose = random_object_pose(self.model, "box_body", sample_annulus_xy(RADIUS_MIN, RADIUS_MAX))
...
__all__ = ["SpotBoxUpright", "SpotBoxUprightConfig"]
